...
1.\textbf{Python}(7 years)   2.\textbf{SQL}(4,5 years) 3. \textbf{ETL}(4 years) 4.\textbf{AWS}(5 years) 5.\textbf{Docker}(5 years) 6.\textbf{Databricks/Spark}(3,5 years) \\7.\textbf{NodeJS/ReactJS/Typescript}(2 years)\\
...
\begin{entrylist}
	\entry
		...
		{Sr Data Engineer}
		{Kake}
		{ At Kake I'm allocated on an external client that works in entertainment field. Within this team I'm responsible for:\\
  - maintain the legacy ingestion platform \\
  - maintain pipelines creating bronze, silver and gold tables in Databricks\\
  - architect the new platform together with the rest of the team\\
  - propose solutions for data governance and data mesh approach
\\\texttt{Scala}\slashsep\texttt{Spark}\slashsep\texttt{DeltaLake}\slashsep\texttt{AWS}\slashsep\texttt{Kafka}\slashsep\texttt{Debezium}\slashsep\texttt{Databricks}}
	\entry
...
\\\texttt{Tableau}\slashsep\texttt{SQL}\slashsep\texttt{Airflow}\slashsep\texttt{AWS}\slashsep\texttt{Docker}\slashsep\texttt{Python}\slashsep\texttt{Data Modeling}\slashsep\texttt{DataDog}\slashsep\texttt{NodeJs}}
	\entry
		...
		{Fullstack/MLOps Software Engineer}
		{NextCam}
		{- Design and build the MLOps fullstack application for Nextcam's main product, improving the performance of training of new models and tunning of old ones\\
